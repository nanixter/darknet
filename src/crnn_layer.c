...
layer make_crnn_layer(int batch, int h, int w, int c, int hidden_filters, int output_filters, int steps, ACTIVATION activation, int batch_normalize, cudaStream_t *stream)
{
    ...
    fprintf(stderr, "\t\t");
    *(l.input_layer) = make_convolutional_layer(batch*steps, h, w, c, hidden_filters, 1, 3, 1, 1,  activation, batch_normalize, 0, 0, 0, stream);
    l.input_layer->batch = batch;

    l.self_layer = malloc(sizeof(layer));
    fprintf(stderr, "\t\t");
    *(l.self_layer) = make_convolutional_layer(batch*steps, h, w, hidden_filters, hidden_filters, 1, 3, 1, 1,  activation, batch_normalize, 0, 0, 0, stream);
    l.self_layer->batch = batch;

    l.output_layer = malloc(sizeof(layer));
    fprintf(stderr, "\t\t");
    *(l.output_layer) = make_convolutional_layer(batch*steps, h, w, hidden_filters, output_filters, 1, 3, 1, 1,  activation, batch_normalize, 0, 0, 0, stream);
    ...
    l.state_gpu = cuda_make_array(l.state, l.hidden*batch*(steps+1), stream);
...
void update_crnn_layer_gpu(layer l, update_args a, cudaStream_t *stream)
{
    update_convolutional_layer_gpu(*(l.input_layer),  a, stream);
    update_convolutional_layer_gpu(*(l.self_layer),   a, stream);
    update_convolutional_layer_gpu(*(l.output_layer), a, stream);
}
...
void forward_crnn_layer_gpu(layer l, network net)
{
    ...
    fill_gpu(l.outputs * l.batch * l.steps, 0, output_layer.delta_gpu, 1, net.stream);
    fill_gpu(l.hidden * l.batch * l.steps, 0, self_layer.delta_gpu, 1, net.stream);
    fill_gpu(l.hidden * l.batch * l.steps, 0, input_layer.delta_gpu, 1, net.stream);
    if(net.train) fill_gpu(l.hidden * l.batch, 0, l.state_gpu, 1, net.stream);
    ...
    for (i = 0; i < l.steps; ++i) {
        ...
        if(l.shortcut){
            copy_gpu(l.hidden * l.batch, old_state, 1, l.state_gpu, 1, net.stream);
        }else{
            fill_gpu(l.hidden * l.batch, 0, l.state_gpu, 1, net.stream);
        }
        axpy_gpu(l.hidden * l.batch, 1, input_layer.output_gpu, 1, l.state_gpu, 1, net.stream);
        axpy_gpu(l.hidden * l.batch, 1, self_layer.output_gpu, 1, l.state_gpu, 1, net.stream);
...
void backward_crnn_layer_gpu(layer l, network net)
{
    ...
    for (i = l.steps-1; i >= 0; --i) {
        copy_gpu(l.hidden * l.batch, input_layer.output_gpu, 1, l.state_gpu, 1, net.stream);
        axpy_gpu(l.hidden * l.batch, 1, self_layer.output_gpu, 1, l.state_gpu, 1, net.stream);
        ...
        copy_gpu(l.hidden*l.batch, self_layer.delta_gpu, 1, input_layer.delta_gpu, 1, net.stream);
        if (i > 0 && l.shortcut) axpy_gpu(l.hidden*l.batch, 1, self_layer.delta_gpu, 1, self_layer.delta_gpu - l.hidden*l.batch, 1, net.stream);
