...
int main(int argc, char* argv[])
{
	...
	while(demuxer.Demux(&compressedFrame, &compressedFrameSize, &dts)) {
		...
		cudaMallocManaged(&compressedFrameDevice, compressedFrameSize);
		...
		cudaMallocManaged(&decompressedFrameDevice, inWidth*inHeight*4);
...
*/
		...
		cudaMallocManaged(&decompressedFrameRGBDevice, inWidth*inHeight*sizeof(float3));
...
*/
		...
		cudaMallocManaged(&scaledFrameNoPad, noPadWidth*noPadHeight*sizeof(float3));
		...
		cudaMallocManaged(&scaledFramePadded, netWidth*netHeight*sizeof(float3));
		...
		cudaMallocManaged(&scaledPaddedPlanarS, netWidth*netHeight*sizeof(float)*3);
...
//			}
		...
		if (numObjects >0) {
			...
			cudaMallocManaged(&boundingBoxesDevice, numObjects*sizeof(float4));
